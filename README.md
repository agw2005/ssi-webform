...
Additionally, you need a snapshot of the `webformdb` database as a `.bak` file,
rename it to `backup.bak`, and place it inside the `/database` workspace. Please
contact the staff at the MIS section of FTI if you need a snapshot.
...
Deployment of the application already assumes that the `webformdb` database
already exists on port 1433.
...
7. In the same directory as the server's executable, copy the `/public`
   directory from the `/server` workspace to that directory.
8. Configure a systemd service to run that executable in the background. Check
   out this
