  ...
  return (
    <div className="h-8 lg:h-9 xl:h-10 | flex">
      ...
      <input
        ...
        name={name}
        id={id}
        className={`text-xs lg:text-sm xl:text-base | flex-1 px-4 text-center border ${resolveColorMappings(variant, "input")} bg-white/50 outline-none w-24`}
        value={currentPage}
        onChange={onInputChangeHandler}
      />
      <div
        className={`text-xs lg:text-sm xl:text-base | font-bold h-full rounded-r-xl justify-self-center border flex items-center px-2 border-r-0 ${resolveColorMappings(variant, "label")} text-white select-none`}
      >
        out of {totalPages}
      </div>
    </div>
